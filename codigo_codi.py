...
def extruturar_dieta(id_dieta):
    conn = conectar_bd()
    cursor = conn.cursor()
    cursor.execute('SELECT id,nome,calorias_totais,criador,email WHERE id_dieta = %s AND apagado = nao',(id_dieta,))
    dados  = cursor.fetchall()
    conn.close()
    conn = conectar_bd()
    cursor = conn.cursor()
    cursor.execute('SELECT id,nome,horario,typo,id_alimento,quantia,preparo WHERE id_dieta = %s',(id_dieta,))
    dados  = cursor.fetchall()
    conn.close()
